...
import "./MarketDeployer-v1_1.sol";
...
/**
 * @title SportsbookFactory v1.6 (pairs with SportsbookMarket v1.11)
 *
 * WHAT'S NEW IN v1.6:
 *   - Nothing in this contract. v1.6 exists only to pair with SportsbookMarket
 *     v1.11 (which adds claimPayoutFor) via MarketDeployer v1.1. The constructor
 *     and createMarket()/createMarketWithBounds() signatures, MarketCreated and
 *     every view are UNCHANGED from v1.5, so the bot needs no change.
 *
 * @title SportsbookFactory v1.5 (pairs with SportsbookMarket v1.10)
 *
 * WHAT'S NEW IN v1.5:
 *   - createMarket() / createMarketWithBounds() take an explicit
 *     `protocolSeed` (per side, USDC base units). 0 = seedless market.
 *     THIS IS A BREAKING ABI CHANGE for market-creation callers (the
 *     market-opener bot). Read paths are unaffected.
 *   - MarketCreated and getMarketInfo() are deliberately UNCHANGED (R-2),
 *     so indexers and the web app need no ABI update. A market's seed is
 *     readable via market.PROTOCOL_SEED().
 *   - MAX_SEED caps the per-side seed at 1000 USDC — the seed is pulled from
 *     the caller and returned at settlement, so a fat-fingered value would
 *     otherwise strand real capital for the life of the market.
 *
...
contract SportsbookFactory is Ownable {
    ...
    // Holds SportsbookMarket's creation bytecode — see MarketDeployer.
    // Pinned at construction and never changeable: swapping it would let the
    // factory mint markets running different code under the same address.
    MarketDeployer public immutable deployer;
    ...
    // Per-side protocol seed cap (USDC base units, 6dp). The seed is the
    // creator's own capital, locked until settlement and then returned; it is
    // excluded from `distributable` and never dilutes a winner's payout.
    uint256 public constant MAX_SEED = 1000e6;
    ...
    constructor(address _usdc, address _oo, address _deployer) {
        ...
        if (_deployer == address(0)) revert InvalidDeployerAddress();
        usdc     = _usdc;
        oo       = _oo;
    ...
    function createMarket(
        ...
        return _createMarket(gameId, oracleZ, DEFAULT_SPREAD_MAX, DEFAULT_SPREAD_MIN, defaultFeePercent, settlementIdentifier, protocolSeed);
    }
    ...
    function createMarketWithBounds(
        ...
        return _createMarket(gameId, oracleZ, spreadMax, spreadMin, defaultFeePercent, settlementIdentifier, protocolSeed);
    }
    ...
    function _createMarket(
        ...
        if (protocolSeed > MAX_SEED) revert SeedTooLarge();

        // Deploy new market — fee, identifier and seed locked at creation time.
        // The `new` lives in MarketDeployer so this factory's runtime bytecode
        // does not carry the market's ~17 KB of creation code (EIP-170).
        // deploy() returns a market already owned by this factory.
        SportsbookMarket market = SportsbookMarket(
            deployer.deploy(usdc, oo, spreadMax, spreadMin, feePercent, identifier, protocolSeed)
        );

        // A seedless market moves no capital: skip the pull and the approval
        // dance entirely rather than round-tripping a zero-value transferFrom.
        if (protocolSeed > 0) {
            // H-3 FIX: transferFrom return value checked (custom error)
            uint256 seedAmount = protocolSeed * 2;
