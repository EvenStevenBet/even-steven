...
Even Steven smart contracts (SportsbookMarket v1.7, SportsbookFactory v1.2) were audited across four rounds in March–April 2026.

---

## Audit Process

Four full audit rounds were conducted using Claude Opus with structured security prompts. Each round focused on incremental changes from the previous version. All findings were addressed before progression to the next round.
...
| Carried | Medium | getMarketEV() return value misleadingly named on minority side | Resolved — renamed to `balancedPayout` with corrected NatSpec |
...
### Round 4 (custom error migration delta)

| ID | Severity | Title | Status |
|---|---|---|---|
| — | — | SportsbookMarket migrated from require() strings to custom errors | Confirmed correct — gas reduction, no behavioral changes |
| — | — | Factory intentionally retains require() strings | Confirmed — factory is not size-constrained, strings aid debugging |
| — | — | Custom error coverage completeness review | Confirmed — all revert paths converted, no gaps |
...
**Round 4 — Custom Error Migration**
SportsbookMarket v1.7 was migrated from `require(condition, "string")` to Solidity custom errors (`revert CustomError()`) to reduce bytecode size for mainnet deployment. The audit confirmed: all revert paths were converted, no behavioral changes introduced, and the factory's continued use of `require()` strings is intentional (factory is not size-constrained; strings aid operator debugging).
...
| No Foundry test suite | Medium | Manual testing only. Full test suite recommended before significant value at risk. |

---

## Auditors

Audited by Claude Opus (Anthropic) using structured security prompts across four rounds. Not a formal third-party audit. A professional audit from a dedicated smart contract security firm is recommended before significant value is at risk on mainnet.
